...
class Job:
    ...
    def getJobDynamic(self, s):
        ...
        self.location = prep(s.div.find(
            'span', {"class": "location-mobile"}).text)
        self.details = prep(s.div.find('div', {"class": "job-details"}).text)
        self.tags, self.jobType, self.expLevel, self.exp = self.getTagsTime(
            self.details)
        self.jobLink = s.div.find(
            'a', {"class": "mobile-job-link"}).attrs['href']
    ...
    def getTagsTime(self, detailsElement):
        ...
        jobTypeWordList = {'Internship', 'Full Time',
                           'Part Time', 'Work From Home', 'Freelance / Project'}
        ExpWordList = {'Entry Level', 'Manager',
                       'Experienced', 'Senior Management', 'Student'}
